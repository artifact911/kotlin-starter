    ...
    val isOld - инициализируется при создании объекта. И если бы даже она была var, то ее значение 
     ...
     getter для isOld() и тогда он будет работать правильно, даже если isOld val
        ...
        class Cat(val name: String, var age: Int, val weight: Float) {
            ...
            fun printInfo() = println("Кличка: $name, Возраст: $age, Вес: $weight")
6. Приватный конструктор у класса указывается в сигнатуре объявления класса private constructor() :
...
Синглтон в Котлин реализован очень просто:
Создаем класс, и вместо слова class в объявлении метода указываем object

Теперь это глобальная переменная к которой можно обращаться отовсюду

### lateinit

НЕ Рекомендуется юзать.

это ключевое слово в объявлении переменной говорит, что у этой переменной поздняя инициализация.
Тогда компилятор позволит ее использовать, но если она не была проинициалицирована. то мы получим ошибку
